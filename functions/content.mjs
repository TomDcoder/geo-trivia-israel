...
export default async (req, context) => {
  ...
  // Two ways in: a Netlify Identity session, or the shared ADMIN_PASSWORD.
  ...
  const passOk = !!expected && supplied === expected;
  const authed = !!identityUser || passOk;

  if (req.method === "GET") {
    const url = new URL(req.url);
    if (url.searchParams.get("auth") === "1") {
      return json({ authOk: authed, passwordConfigured: !!expected });
    }
    const doc = await store.get(KEY, { type: "json" });
    if (!doc) return json({ empty: true, revision: 0, passwordConfigured: !!expected });
    return json(doc);
  }

  if (req.method === "PUT" || req.method === "POST") {
    if (!authed) return json({ error: "unauthorized", message: "נדרשת התחברות כדי לשמור תוכן" }, 401);
    ...
    const who = identityUser
      ? (identityUser.email || (identityUser.user_metadata && identityUser.user_metadata.full_name) || "עורך")
      : "עורך";
    const doc = {
      revision: prevRev + 1,
